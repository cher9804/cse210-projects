...
class Program
{
    static void Main(string[] args)
    {
        ...
        Scripture scripture = new Scripture(newreference, "Trust in the Lord with all thine heart; and lean not unto thine own understanding. In all thy ways acknowledge him, and he shall direct thy paths.");

        Console.WriteLine("Press ENTER to continue ot type 'quit' to finish: ");
        Console.ReadLine();
        ...
        while (true)
        {
            ...
            {
                Console.WriteLine("You completed the remembered the whole verse.");
                break;
            }
            Console.WriteLine();
            Console.WriteLine("Press ENTER to hide random words, type 'HINT' to get some hints or 'quit' to exit the program.");
            string input = Console.ReadLine();
            if (input.ToUpper() == "HINT")
            {

                scripture.Display(showHint: true);
            ...
            {
                scripture.HideRandomWords(3, hideOnlyVisible: true);
